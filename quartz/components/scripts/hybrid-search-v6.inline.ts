...
const NEIGHBOR_CONTEXT_CHARS = 850
...
function queryModel(query: string): QueryModel {
  ...
  const statuteWords = new Set(statutes.flatMap((group) => group.flatMap((phrase) => rawTokens(phrase))))
  if (/\bcla\b/i.test(query)) statuteWords.add("cla")
  statuteWords.add("act")

  const discriminatorTerms = coreTerms.filter((term) => term !== section && !statuteWords.has(term))
...
function doctrineIntentRelation(
  text: string,
  model: QueryModel,
  doctrinePositions: number[],
  intentPositions: number[],
): number {
...
function sameSectionContext(a: LexicalChunk | undefined, b: LexicalChunk): boolean {
  if (!a) return false
  const aPath = a.hPath.filter(Boolean)
  const bPath = b.hPath.filter(Boolean)
  if (aPath.length === 0 || bPath.length === 0) return true
  return aPath[0] === bPath[0]
}

function adjacentWindow(chunks: LexicalChunk[], index: number): LexicalChunk {
  const current = chunks[index]
  const previous = chunks[index - 1]
  const next = chunks[index + 1]
  const parts: string[] = []

  if (sameSectionContext(previous, current)) {
    parts.push(previous.text.slice(-NEIGHBOR_CONTEXT_CHARS))
  }
  parts.push(current.text)
  if (sameSectionContext(next, current)) {
    parts.push(next.text.slice(0, NEIGHBOR_CONTEXT_CHARS))
  }
...
function centeredSnippet(
  ...
  const terms = focusTerms && focusTerms.length > 0 ? focusTerms : meaningfulTerms(query)
  const positions = terms
    .flatMap((term) => termPositions(normalized, term))
    .sort((a, b) => a - b)
  ...
  let start = Math.max(0, approximateRawCenter - Math.floor(maxChars * 0.42))
...
function betterCandidate(model: QueryModel, candidate: RankedHit, best: RankedHit): boolean {
  const discriminatorSensitive = model.statutes.length > 0 && model.discriminatorTerms.length > 0
  if (discriminatorSensitive) {
    const candidateFull = candidate.evidence.discriminatorCoverage >= 0.99
    const bestFull = best.evidence.discriminatorCoverage >= 0.99
    if (candidateFull !== bestFull) return candidateFull
    if (candidate.evidence.discriminatorCoverage !== best.evidence.discriminatorCoverage) {
      return candidate.evidence.discriminatorCoverage > best.evidence.discriminatorCoverage
    }
  }
...
function rankLexical(query: string, docs: LexicalDocument[]): RankedHit[] {
  ...
  const focusTerms = model.discriminatorTerms.length > 0 ? model.discriminatorTerms : model.coreTerms
  ...
  for (const doc of docs) {
    ...
    for (let index = 0; index < doc.chunks.length; index++) {
      const sourceChunk = doc.chunks[index]
      const windowChunk = adjacentWindow(doc.chunks, index)
      const evidence = passageEvidence(model, doc.title, windowChunk)
      ...
      const candidate: RankedHit = {
        ...
        hPath: sourceChunk.hPath,
        snippet: centeredSnippet(query, windowChunk.text, SNIPPET_CHARS, focusTerms),
        evidence,
      }

      if (!best || betterCandidate(model, candidate, best)) best = candidate
    }
    ...
    const fullDiscriminatorDocMatch =
      model.discriminatorTerms.length === 0 || discriminatorDocCoverage >= 0.99
    const fullDiscriminatorPassageMatch =
      model.discriminatorTerms.length === 0 || best.evidence.discriminatorCoverage >= 0.99
    const statuteNeedsDiscriminators = model.statutes.length > 0 && model.discriminatorTerms.length > 0
    const canRescue =
      !statuteNeedsDiscriminators ||
      (fullDiscriminatorDocMatch && fullDiscriminatorPassageMatch)

    if (
      canRescue &&
      model.coreTerms.length >= 3 &&
      docCoverage >= 0.75 &&
      best.evidence.tier < 7
    ) {
      best = {
        ...best,
        evidence: {
          ...
          tier: Math.max(
            best.evidence.tier,
            model.statutes.length > 0 && fullDiscriminatorDocMatch ? 7 : docCoverage === 1 ? 6 : 5,
          ),
...
document.addEventListener("nav", async (event: CustomEventMap["nav"]) => {
  ...
  searchLayout.dataset.engine = "hybrid-v6"
  console.info("[Quartz search] hybrid-v6 adjacent-window ranking active")
      ...
      for (const [normalized, sem] of semantic) {
        ...
        if (existing) {
          ...
          if (existing.evidence.tier < 5) {
            existing.evidence = {
              ...existing.evidence,
              score: Math.min(1, 0.7 * existing.evidence.score + 0.3 * sem.score),
            }
          }
        } else if (strongDeterministic < 7 || sem.score >= 0.78) {
